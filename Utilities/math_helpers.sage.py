...
# This file was *autogenerated* from the file math_helpers.sage
from sage.all_cmdline import *   # import sage library

_sage_const_25 = Integer(25); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_4 = Integer(4); _sage_const_10 = Integer(10)
...
def Y_lm_test(l=None, m=None):
    return EEE.scalar_field(
        sqrt((_sage_const_2 *l + _sage_const_1 )*factorial(l - m)/(_sage_const_4 *pi*factorial(l + m)))
...
def a_lm_E_long_wavelength(l, m, Q_static, Q_induced=_sage_const_0 ):
    ...
    """
    return c*k**(l + _sage_const_2 )/(i*factorial2(_sage_const_2 *l + _sage_const_1 ))*sqrt((l + _sage_const_1 )/l)*(Q_static + Q_induced) if l != _sage_const_0  else _sage_const_0 
...
def a_lm_M_long_wavelength(l, m, M_current, M_intrinsic):
    ...
    """
    return i*k**(l+_sage_const_2 )/factorial2(_sage_const_2 *l+_sage_const_1 )*sqrt((l+_sage_const_1 )/l)*(M_current + M_intrinsic) if l != _sage_const_0  else _sage_const_0 
...
def E_wavefront_lm(l, m, E):
    ...
    return -k/Z_0/r**_sage_const_2 /sqrt(l*(l+_sage_const_1 ))*EEE.scalar_field(
        integral_coord_region(
            apply_to(conjugate, Y_lm_jackson(l, m))*r_vec.dot(E),
            {r: r, th: [_sage_const_0 , pi], ph: [_sage_const_0 , _sage_const_2 *pi]}
        )
    )
...
def E_dipole_fields(moment, direction: Optional[Direction] = None) -> Fields:
    ...
    >>> 
    """
    A = (-i*mu_0*k*c*e**(i*k*r)*moment/(_sage_const_4 *pi*r)*z_hat)
    H = H_from_A_free_space(A)
...
def multipole_power_cross_section_pure(l, m, a, k=k, Z_0=Z_0):
  ...
  See Jackson 9.151.
    ...
    :param Z_0: (Optional) wave impedance.  If not provided, the variable 'Z_0' is used.
    """
    return Z_0/(_sage_const_2 *k**_sage_const_2 )*norm(a + _sage_const_0 *i)*complex_norm(X_lm_jackson(l, m))
...
def test_equality_radial(qty1, qty2, r_min=_sage_const_0 , r_max=_sage_const_10 ):
    ...
    """
    exprs = [complex_norm(qty1 - qty2).expr(), _sage_const_1  - complex_norm(qty1/qty2).expr()]
    ret = None
